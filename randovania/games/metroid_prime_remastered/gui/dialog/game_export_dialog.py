...
def romfs_validation(line: QtWidgets.QLineEdit):
    ...
    return not all(
        p.is_file()
        for p in [
            path.joinpath("KernelMPRT.pak"),
            path.joinpath("Preload", "DuplicateData.pak"),
            path.joinpath("UI", "MPR1", "UIMP1.pak"),
            path.joinpath("Worlds", "MP1", "universeRoomMP1.pak"),
        ]
    )


class MP1RGameExportDialog(GameExportDialog, Ui_RemasteredGameExportDialog):
    ...
    def get_path_to_ryujinx(self) -> Path:
        ryujinx_path_tuple = ("Ryujinx", "mods", "contents", "010012101468C000")
    ...
    def __init__(self, options: Options, patch_data: dict, word_hash: str, spoiler: bool, games: list[RandovaniaGame]):
        ...
        # TODO: ryujinx support
        self.ryujinx_radio.setEnabled(False)

        # TODO: FTP support (maybe, upload will take a long time)
        self.tab_ftp.setEnabled(False)
    ...
    @property
    def target_platform(self) -> MP1RModPlatform:
        if self.atmosphere_radio.isChecked():
            return MP1RModPlatform.ATMOSPHERE
        else:
            return MP1RModPlatform.RYUJINX
    ...
    def update_per_game_options(self, per_game: PerGameOptions) -> MP1RPerGameOptions:
        assert isinstance(per_game, MP1RPerGameOptions)
    ...
    def get_game_export_params(self) -> MP1RGameExportParams:
        ...
        spoiler_output = spoiler_path_for(self.auto_save_spoiler, output_path)
    ...
    # Shared Code from Metroid Dread
    ...
    def _on_update_target_platform(self):
        ...
        self.output_tab_widget.setTabVisible(
            ...
            target_platform == MP1RModPlatform.ATMOSPHERE and platform.system() == "Windows",
        )
        self.output_tab_widget.setTabVisible(
            self.output_tab_widget.indexOf(self.tab_ftp), target_platform == MP1RModPlatform.ATMOSPHERE
        )
        self.output_tab_widget.setTabVisible(
            self.output_tab_widget.indexOf(self.tab_ryujinx),
            target_platform == MP1RModPlatform.RYUJINX and supports_ryujinx(),
        )
    ...
    def get_sd_card_output_path(self) -> Path:
        ...
        if self.sd_mod_manager_check.isChecked():
            return root_path.joinpath("mods", "Metroid Prime Remastered", f"Randovania {self._word_hash}")
        else:
    ...
    def _get_ftp_internal_path(self):
        return self._options.internal_copies_path.joinpath("mp1r", "contents")
    ...
    def _on_custom_path_button(self):
        output_file = prompt_for_output_directory(self, "PrimeRemasterRandovania", self.custom_path_edit)
